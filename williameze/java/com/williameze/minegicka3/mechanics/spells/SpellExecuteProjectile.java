...
public class SpellExecuteProjectile extends SpellExecute
{
	...
	else if (s.castType == CastType.Self)
	{
	    ...
	    {
		double manaToConsume = s.countElements() * 75 * s.getManaConsumeRate();
		if (consumeMana(s, manaToConsume, true, true, 3) > 0)
		{
		    if (s.hasElement(Element.Earth))
		    {
			for (int a = 0; a < s.countElement(Element.Earth); a++)
			{
			    EntityBoulder bld = new EntityBoulder(caster.worldObj);
			    bld.setSpell(s);
			    bld.setPosition(caster.posX + (rnd.nextDouble() - 0.5) * bld.width * 4, caster.posY + 8 + bld.width,
				    caster.posZ + (rnd.nextDouble() - 0.5) * bld.width * 4);
			    bld.motionY = -1;
		    ...
		    {
			for (int a = 0; a < s.countElements() * 4 + 4; a++)
			{
			    EntityIcicle ici = new EntityIcicle(caster.worldObj);
			    ici.setSpell(s);
			    ici.setPosition(caster.posX + (rnd.nextDouble() - 0.5) * 2 * s.countElements(), caster.posY + 8,
				    caster.posZ + (rnd.nextDouble() - 0.5) * 2 * s.countElements());
			    ici.motionY = -1.5;
			    caster.worldObj.spawnEntityInWorld(ici);
			}
		    }
		}
	    }
