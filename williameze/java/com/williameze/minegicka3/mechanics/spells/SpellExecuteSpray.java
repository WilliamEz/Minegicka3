...
public class SpellExecuteSpray extends SpellExecute
{
	...
	else
	{
	    ...
	    double power = Math.min(Math.pow(count * s.getPower(), 0.4) * 1.25D, 5);
	    ...
	    Vector pos = (new Vector(caster.posX, caster.posY + caster.getEyeHeight(), caster.posZ)).add(dir.multiply(caster.width / 2));
	    if (s.castType == CastType.Area)
	    {
	    ...
	    {
		pos = new Vector(caster.posX + (rnd.nextDouble() - 0.5) * count, caster.posY + caster.getEyeHeight() + 3, caster.posZ
			+ (rnd.nextDouble() - 0.5) * count);
